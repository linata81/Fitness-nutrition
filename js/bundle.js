...
function cards() {
  class MenuCard {
    ...
    changeToUAN() { //метод для конвертации долларов в гривны
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResource)('http://localhost:3000/menu')
    ...
    /* вариант без формирования классов, генерация верстки на лету */
...
function modal(triggerSelector, modalSelector, modalTimerId){
  ...
  modalTrigger.forEach(btn => {
    btn.addEventListener('click', () => openModal(modalSelector, modalTimerId)); //оборачиваем ф-цию в стрелочную, иначе из-за атрибута она вызовется до события клик
  });
  ...
  //открытие модалки, когда пользователь долистал до конца страницы
...
function timer(id, deadline){
  ...
  */
  ...
  function setClock(selector, endtime){
    ...
    updateClock(); //чтобы 1 раз без задержки зангрузились данные на счетчик
...
const postData = async (url, data) => {
  ...
  return  await res.json(); //ф-ция возвращает промис, кот потом обработаем then
};
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  //ч/з 50 сек на странице открыть модалку
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.openModal)('.modal', modalTimerId), 50000);

  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_0__.default)('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active');
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.default)('[data-modal]','.modal', modalTimerId);
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__.default)('.timer', '2021-12-10');
